...
REPO_DIR="/Users/zhangheng/.openclaw/workspace/next-static-page"
CHAT_TO="+8613225036298"
...
# Simple deterministic "hourly" iteration: add a new easter-egg page and wire it up.
PAGE_FILE="src/pages/lab.tsx"
...
# Update home page to link Lab if not already
if ! grep -q 'href="/lab"' src/pages/index.tsx; then
  perl -0777 -i -pe 's/(<div className=\"cta\">\s*\n\s*<Link className=\"btn primary\" href=\"\/notes\">\n\s*Open Notes\n\s*<\/Link>\n)/$1            <Link className=\"btn ghost\" href=\"\/lab\">\n              Open Lab\n            <\/Link>\n/sg' src/pages/index.tsx
fi
...
if ! git diff --cached --quiet; then
  git commit -m "chore: hourly iteration ${STAMP}"
  git push
  npx vercel --prod --yes
fi

# Optional: send a short notification (best-effort) via OpenClaw message
openclaw message send --channel whatsapp --target "$CHAT_TO" --message "[next-static-page] Hourly iteration shipped at ${STAMP} on ${BRANCH}. Deployed: https://next-static-page.vercel.app" >/dev/null 2>&1 || true
